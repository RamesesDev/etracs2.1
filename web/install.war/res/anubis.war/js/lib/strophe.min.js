...
var Notifier = function( sessionid, url ) {
	...
	var onMessage = function( xmlmsg ) {
		try {
			var type = xmlmsg.getAttribute('type');
			var elems = xmlmsg.getElementsByTagName('body');
	...
	var onConnect = function(status) {		
		...
		else if (status == Strophe.Status.CONNECTED || status == Strophe.Status.ATTACHED) 
		{
			...
			connection.send($pres().c('priority').t("1").tree());
		} 
		else if( status == Strophe.Status.AUTHFAIL ) 
		{
			log('login failed..');
		} 
		else if( status == Strophe.Status.CONNECTING ||  status == Strophe.Status.DISCONNECTING || status== Strophe.Status.AUTHENTICATING) {} 
		else 
		{
			log('[Status : '+status+']failed to connect to the server');
		}
	}
	
	this.connect = function() {
		var svc = ProxyService.lookup('NotifierService');
		
		var handler = function( result ) {
			try {
				connection = new Strophe.Connection(self.url);
				connection.connect( result.uid, result.pwd, onConnect ); 
				//connection.attach( result.uid, result.sid, result.rid, onConnect );
			}
			catch(e) {
				log( e );
			}
		...
		svc.connect( this.sessionid, handler );
	}
